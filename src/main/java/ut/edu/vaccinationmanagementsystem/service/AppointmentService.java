...
public class AppointmentService {
    ...
    public Appointment bookAppointment(BookAppointmentDTO dto) {
        ...
        if (userIdToCheck != null) {
            ...
            for (Appointment existingAppt : existingAppointments) {
                ...
                if (existingAppt.getVaccine().getId().equals(vaccine.getId())) {
                ...
                // Nếu vaccine khác nhau → check incompatibility
                Optional<VaccineIncompatibility> incompatibilityOpt = 
                        vaccineIncompatibilityRepository.findIncompatibility(
                                existingAppt.getVaccine().getId(),
                                vaccine.getId()
                        );
                
                if (incompatibilityOpt.isPresent()) {
                    VaccineIncompatibility incompatibility = incompatibilityOpt.get();
                    throw new RuntimeException(
                            String.format(
        ...
        appointment.setStatus(AppointmentStatus.PENDING); // Trạng thái chờ xác nhận
    ...
    private void validateVaccineCompatibility(Vaccine newVaccine, LocalDate appointmentDate, 
        ...
        for (Appointment existingAppt : recentAppointments) {
            ...
            // 1. Check vaccine incompatibility (vaccine không tương thích)
            Optional<VaccineIncompatibility> incompatibilityOpt = 
                    vaccineIncompatibilityRepository.findIncompatibility(
                            existingVaccine.getId(),
                            newVaccine.getId()
                    );
    ...
    public Appointment cancelAppointment(Long appointmentId, User currentUser) {
        ...
        if (appointment.getStatus() != AppointmentStatus.PENDING) {
            throw new RuntimeException(
                String.format("Chỉ có thể hủy lịch hẹn khi trạng thái là 'Chờ xác nhận'. Trạng thái hiện tại: %s", 
    ...
    public void deleteAppointmentWhenPaymentFailed(Long appointmentId) {
        ...
        // Chỉ xóa nếu payment method là VNPAY và status là PENDING
        if (appointment.getPayment() != null && 
            ...
            throw new RuntimeException("Chỉ có thể xóa appointment khi payment method là VNPAY và status là PENDING");
        }
    }
